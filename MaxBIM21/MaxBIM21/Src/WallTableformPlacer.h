...
namespace namespaceWallTableform {
	...
	char objTypeStr[14][32] = { "없음", "유로폼", "합판", "각재", "휠러스페이서", "테이블폼A", "테이블폼B", "테이블폼C", "인코너판넬(L)", "아웃코너판넬(L)", "아웃코너앵글(L)", "인코너판넬(R)", "아웃코너판넬(R)", "아웃코너앵글(R)" };
		...
		PlacingZone()
		{
			...
			for (int i = 0; i < 10; i++) {
				for (int j = 0; j < 50; j++) {
					...
					this->cellsBasic[i][j].nCellsVer = 0;
					for (int m = 0; m < 10; m++) {
						this->cellsBasic[i][j].cellHorLen[m] = 0.0;
						this->cellsBasic[i][j].cellVerLen[m] = 0.0;
					}
					...
					this->cellsExtra[i][j].nCellsVer = 0;
					for (int m = 0; m < 10; m++) {
						this->cellsExtra[i][j].cellHorLen[m] = 0.0;
						this->cellsExtra[i][j].cellVerLen[m] = 0.0;
					}
		...
		void initCells()
		{
			...
			for (int i = 0; i < 10; i++) {
				for (int j = 0; j < 50; j++) {
					...
					this->cellsBasic[i][j].nCellsVer = this->nCellsVerBasic;
					...
					for (int m = 0; m < 10; m++) {
						this->cellsBasic[i][j].cellVerLen[m] = 1.200;
					}
					...
					this->cellsExtra[i][j].nCellsVer = this->nCellsVerBasic;
					...
					for (int m = 0; m < 10; m++) {
						this->cellsExtra[i][j].cellVerLen[m] = 1.200;
					}
				}
			}
		}
	...
	short POPUP_OBJ_WIDTH_START_INDEX;			// 객체 너비 (팝업컨트롤)
	short EDITCONTROL_OBJ_WIDTH_START_INDEX;	// 객체 너비 (Edit컨트롤)
	short BUTTON_ADD_COLUMN;					// 열 방향 객체 추가 버튼
	short BUTTON_DEL_COLUMN;					// 열 방향 객체 삭제 버튼
	short POPUP_OBJ_HEIGHT_START_INDEX;			// 객체 높이 (팝업컨트롤)
	...
	short DGCALLBACK handler1(short message, short dialogID, short item, DGUserData /* userData */, DGMessageData /* msgData */) {
		...
		case DG_MSG_INIT:
			...
			posX = 205; posY = 180;
			sizeX = 100; sizeY = 100;
			for (short i = 0; i < placingZone.nCellsVerBasic; i++) {
			...
			// 객체 타입 선택 버튼 표시
			posX = 205; posY = 180 + (placingZone.nCellsVerBasic * sizeY) + 5;
			for (short i = 0; i < placingZone.nCellsHor; i++) {
			...
			// 객체 너비 (팝업컨트롤)
			posX = 205; posY = 180 + (placingZone.nCellsVerBasic * sizeY) + 35;
			for (short i = 0; i < placingZone.nCellsHor; i++) {
			...
			// 객체 높이 (팝업컨트롤)
			posX = 100; posY = 180 + (placingZone.nCellsVerBasic * sizeY) - 63;
			for (short i = 0; i < placingZone.nCellsVerBasic; i++) {
			...
			// 다이얼로그 창 크기 조절
			DGSetDialogSize(dialogID, DG_CLIENT, 205 + (placingZone.nCellsHor * sizeX) + 100, 180 + (placingZone.nCellsVerBasic * sizeY) + 100, DG_TOPLEFT, true);
			...
			// 기본값 설정: 너비 팝업컨트롤
			for (int i = POPUP_OBJ_WIDTH_START_INDEX; i <= POPUP_OBJ_WIDTH_START_INDEX + placingZone.nCellsHor; i++) {
				...
				DGPopUpSelectItem(dialogID, i, DG_POPUP_TOP+1);
			}
		...
		case DG_MSG_CHANGE:
			// ... 테이블폼 방향 라디오 버튼
				// ... 기존 팝업 컨트롤의 내용물을 전부 지우고 다시 채움 (테이블폼, 유로폼)
