...
namespace wallTableformPlacerDG {
	WallTableformPlacingZone		placingZone;		// 기본 벽면 영역 정보
	InfoWall						infoWall;			// 벽 객체 정보
	insulElemForWallTableform	insulElem;			// 단열재 정보
	...
	GS::Array<API_Guid>	elemList_Front;	// 그룹화를 위해 생성된 결과물들의 GUID를 전부 저장함 (앞면)
	GS::Array<API_Guid>	elemList_Back;	// 그룹화를 위해 생성된 결과물들의 GUID를 전부 저장함 (뒷면)
	GS::Array<API_Guid>	elemList_Front_Add;	// 그룹화를 위해 생성된 결과물들의 GUID를 전부 저장함 (앞면)
	GS::Array<API_Guid>	elemList_Back_Add;	// 그룹화를 위해 생성된 결과물들의 GUID를 전부 저장함 (뒷면)

	int	clickedIndex;	// 클릭한 버튼의 인덱스
	int	iMarginSide;	// 벽 상단을 채우고자 하는데 현재 진행되는 쪽이 어느 쪽 면인가? (1-낮은쪽, 2-높은쪽)
...
GSErrCode	placeTableformOnWall (void)
{
	...
	// 벽이 1개인가?
	if (nWalls != 1) {
		DGAlert (DG_ERROR, L"오류", L"벽을 1개 선택해야 합니다.", "", L"확인", "", "");
		err = APIERR_GENERAL;
		return err;
	}
	...
	if ((nMorphs < 1) || (nMorphs > 2)) {
		DGAlert (DG_ERROR, L"오류", L"벽을 덮는 모프를 1개 또는 2개를 선택하셔야 합니다.", "", L"확인", "", "");
		err = APIERR_GENERAL;
		return err;
	}
...
FIRST:
	...
	result = DGModalDialog (ACAPI_GetOwnResModule (), 32601, ACAPI_GetOwnResModule (), wallTableformPlacerHandler2, 0);
	...
	if (placingZone->typeLcorner == INCORNER_PANEL) {
		...
		for (xx = 0 ; xx < placingZone->nCellsInVerBasic ; ++xx) {
			elemList_Front.Push (incornerPanel.placeObject (5,
				...
				"hei_s", APIParT_Length, format_string ("%f", (double)placingZone->cells [0].tableInVerBasic [xx] / 1000.0).c_str(),
		...
		if (placingZone->bSingleSide == false) {
			...
			for (xx = 0 ; xx < varEnd ; ++xx) {
				...
				elemList_Back.Push (incornerPanel.placeObject (5,
					...
					"hei_s", APIParT_Length, format_string ("%f", lengthDouble).c_str(),
					"dir_s", APIParT_CString, "세우기"));
		...
		while (remainLengthInt > 0) {
			...
			elemList_Front.Push (outcornerAngle.placeObject (2,
				"a_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(),
			...
			for (yy = 0 ; yy < placingZone->nCellsInVerBasic ; ++yy) {
				if (placingZone->bVertical == true) {
					...
					elemList_Front.Push (euroform.placeObject (5,
						...
						"eu_hei", APIParT_CString, format_string ("%d", placingZone->cells [xx].tableInVerBasic [yy]).c_str(),
					...
					elemList_Front.Push (euroform.placeObject (5,
						"eu_stan_onoff", APIParT_Boolean, "1.0",
						"eu_wid", APIParT_CString, format_string ("%d", placingZone->cells [xx].tableInVerBasic [yy]).c_str(),
				...
				for (yy = 0 ; yy < varEnd ; ++yy) {
					...
					if (placingZone->bVertical == true) {
						...
						elemList_Back.Push (euroform.placeObject (5,
							"eu_stan_onoff", APIParT_Boolean, "1.0",
							"eu_wid", APIParT_CString, format_string ("%d", placingZone->cells [xx].horLen).c_str(),
							"eu_hei", APIParT_CString, format_string ("%d", lengthInt).c_str(),
							"u_ins", APIParT_CString, "벽세우기",
							"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
						moveIn3D ('x', euroform.radAng, (double)placingZone->cells [xx].horLen / 1000.0, &euroform.posX, &euroform.posY, &euroform.posZ);
					} else {
						...
						elemList_Back.Push (euroform.placeObject (5,
							"eu_stan_onoff", APIParT_Boolean, "1.0",
							"eu_wid", APIParT_CString, format_string ("%d", lengthInt).c_str(),
							"eu_hei", APIParT_CString, format_string ("%d", placingZone->cells [xx].horLen).c_str(),
							"u_ins", APIParT_CString, "벽눕히기",
							"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
					}
		...
		if (placingZone->cells [xx].objType == FILLERSP) {
			...
			while (remainLengthInt > 0) {
				...
				elemList_Front.Push (fillersp.placeObject (4,
					...
					"f_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(),
		...
		if (placingZone->cells [xx].objType == PLYWOOD) {
			...
			while (remainLengthInt > 0) {
				...
				elemList_Front.Push (plywood.placeObject (13,
					...
					"p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(),
		...
		if (placingZone->cells [xx].objType == TIMBER) {
			...
			while (remainLengthInt > 0) {
				...
				elemList_Front.Push (timber.placeObject (6,
					...
					"w_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(),
			...
			if (placingZone->cells [idxCell].objType == TABLEFORM) {
				for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++yy) {
					if (placingZone->cells [idxCell].tableInHor [yy] == 600) {
						...
						elemList_Front.Push (blueClamp.placeObject (4, "type", APIParT_CString, "유로목재클램프(제작품v1)", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(), "openingWidth", APIParT_Length, format_string ("%f", 0.048).c_str()));
				...
				if ((plywoodMarginBasic >= 0.010 - EPS) && (plywoodMarginBasic <= 0.020 + EPS)) {
					if (placingZone->cells [idxCell].objType == TABLEFORM) {
						for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++yy) {
							if (placingZone->cells [idxCell].tableInHor [yy] == 600) {
								...
								elemList_Front.Push (blueTimberRail.placeObject (3, "railType", APIParT_CString, "블루목심 4", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
					...
					if (placingZone->cells [idxCell].objType == TABLEFORM) {
						for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++yy) {
							if (placingZone->cells [idxCell].tableInHor [yy] == 600) {
								moveIn3D ('x', blueTimberRail.radAng, 0.150, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
								elemList_Front.Push (blueTimberRail.placeObject (3, "railType", APIParT_CString, "블루목심 1", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
								moveIn3D ('x', blueTimberRail.radAng, 0.450, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
							} else if (placingZone->cells [idxCell].tableInHor [yy] == 500) {
								moveIn3D ('x', blueTimberRail.radAng, 0.150, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
								elemList_Front.Push (blueTimberRail.placeObject (3, "railType", APIParT_CString, "블루목심 1", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
					...
					if (placingZone->cells [idxCell].objType == TABLEFORM) {
						for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++yy) {
							if (placingZone->cells [idxCell].tableInHor [yy] == 600) {
								moveIn3D ('x', blueTimberRail.radAng, 0.150, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
								elemList_Front.Push (blueTimberRail.placeObject (3, "railType", APIParT_CString, "블루목심 3", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
								moveIn3D ('x', blueTimberRail.radAng, 0.450, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
							} else if (placingZone->cells [idxCell].tableInHor [yy] == 500) {
								moveIn3D ('x', blueTimberRail.radAng, 0.150, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
								elemList_Front.Push (blueTimberRail.placeObject (3, "railType", APIParT_CString, "블루목심 3", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
				...
				if (placingZone->cells [idxCell].objType == TABLEFORM) {
					for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++yy) {
						if (placingZone->cells [idxCell].tableInHor [yy] == 600) {
							...
							elemList_Back.Push (blueClamp.placeObject (4, "type", APIParT_CString, "유로목재클램프(제작품v1)", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(), "openingWidth", APIParT_Length, format_string ("%f", 0.048).c_str()));
					...
					if ((plywoodMarginExtra >= 0.010 - EPS) && (plywoodMarginExtra <= 0.020 + EPS)) {
						if (placingZone->cells [idxCell].objType == TABLEFORM) {
							for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++yy) {
								if (placingZone->cells [idxCell].tableInHor [yy] == 600) {
									...
									elemList_Back.Push (blueTimberRail.placeObject (3, "railType", APIParT_CString, "블루목심 4", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
						...
						if (placingZone->cells [idxCell].objType == TABLEFORM) {
							for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++yy) {
								if (placingZone->cells [idxCell].tableInHor [yy] == 600) {
									moveIn3D ('x', blueTimberRail.radAng - DegreeToRad (180.0), 0.150, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
									elemList_Back.Push (blueTimberRail.placeObject (3, "railType", APIParT_CString, "블루목심 1", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
									moveIn3D ('x', blueTimberRail.radAng - DegreeToRad (180.0), 0.450, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
								} else if (placingZone->cells [idxCell].tableInHor [yy] == 500) {
									moveIn3D ('x', blueTimberRail.radAng - DegreeToRad (180.0), 0.150, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
									elemList_Back.Push (blueTimberRail.placeObject (3, "railType", APIParT_CString, "블루목심 1", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
						...
						if (placingZone->cells [idxCell].objType == TABLEFORM) {
							for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++yy) {
								if (placingZone->cells [idxCell].tableInHor [yy] == 600) {
									moveIn3D ('x', blueTimberRail.radAng - DegreeToRad (180.0), 0.150, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
									elemList_Back.Push (blueTimberRail.placeObject (3, "railType", APIParT_CString, "블루목심 3", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
									moveIn3D ('x', blueTimberRail.radAng - DegreeToRad (180.0), 0.450, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
								} else if (placingZone->cells [idxCell].tableInHor [yy] == 500) {
									moveIn3D ('x', blueTimberRail.radAng - DegreeToRad (180.0), 0.150, &blueTimberRail.posX, &blueTimberRail.posY, &blueTimberRail.posZ);
									elemList_Back.Push (blueTimberRail.placeObject (3, "railType", APIParT_CString, "블루목심 3", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
				...
				if ((plywoodMarginBasic > EPS) && (lengthInt > 0)) {
					if (idxCell == 0) {
						if (bFirstPlywood == true)
							elemList_Front.Push (plywood.placeObject (13, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "벽눕히기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", plywoodMarginBasic).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "1.0", "bInverseSogak", APIParT_Boolean, "1.0", "prof", APIParT_CString, "소각", "gap_a", APIParT_Length, format_string ("%f", 0.070).c_str(), "gap_b", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_c", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_d", APIParT_Length, format_string ("%f", 0.0).c_str()));
						else
							elemList_Front.Push (plywood.placeObject (13, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "벽눕히기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", plywoodMarginBasic).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "1.0", "bInverseSogak", APIParT_Boolean, "1.0", "prof", APIParT_CString, "소각", "gap_a", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_b", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_c", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_d", APIParT_Length, format_string ("%f", 0.0).c_str()));
					} else if (idxCell == this->nCellsInHor-1) {
						if (bLastPlywood == true)
							elemList_Front.Push (plywood.placeObject (13, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "벽눕히기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", plywoodMarginBasic).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "1.0", "bInverseSogak", APIParT_Boolean, "1.0", "prof", APIParT_CString, "소각", "gap_a", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_b", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_c", APIParT_Length, format_string ("%f", 0.070).c_str(), "gap_d", APIParT_Length, format_string ("%f", 0.0).c_str()));
						else
							elemList_Front.Push (plywood.placeObject (13, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "벽눕히기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", plywoodMarginBasic).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "1.0", "bInverseSogak", APIParT_Boolean, "1.0", "prof", APIParT_CString, "소각", "gap_a", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_b", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_c", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_d", APIParT_Length, format_string ("%f", 0.0).c_str()));
					} else {
						elemList_Front.Push (plywood.placeObject (13, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "벽눕히기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", plywoodMarginBasic).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "1.0", "bInverseSogak", APIParT_Boolean, "1.0", "prof", APIParT_CString, "소각", "gap_a", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_b", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_c", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_d", APIParT_Length, format_string ("%f", 0.0).c_str()));
			...
			if (plywoodMarginBasic + EPS > 0.0) {
				...
				if ((plywoodMarginBasic >= 0.010 - EPS) && (plywoodMarginBasic <= 0.030 + EPS)) {
					...
					while (remainLengthInt > 0) {
						...
						elemList_Front.Push (timber.placeObject (6, "w_ins", APIParT_CString, "벽세우기", "w_w", APIParT_Length, format_string ("%f", 0.050).c_str(), "w_h", APIParT_Length, format_string ("%f", 0.080).c_str(), "w_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "w_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "torsion_ang", APIParT_Angle, format_string ("%f", 0.0).c_str()));
					...
					while (remainLengthInt > 0) {
						...
						if (addedPlywood >= 1) {
							...
							elemList_Front.Push (plywood1.placeObject (7, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "바닥덮기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", 0.070).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "0.0"));
							moveIn3D ('y', plywood1.radAng, 0.070, &plywood1.posX, &plywood1.posY, &plywood1.posZ);
							moveIn3D ('z', plywood1.radAng, -moveZ, &plywood1.posX, &plywood1.posY, &plywood1.posZ);
						...
						if (addedPlywood >= 2) {
							...
							elemList_Front.Push (plywood2.placeObject (7, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "바닥덮기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", 0.070).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "0.0"));
							moveIn3D ('y', plywood2.radAng, 0.070, &plywood2.posX, &plywood2.posY, &plywood2.posZ);
							moveIn3D ('z', plywood2.radAng, -moveZ, &plywood2.posX, &plywood2.posY, &plywood2.posZ);
						...
						if (addedPlywood >= 3) {
							...
							elemList_Front.Push (plywood3.placeObject (7, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "바닥덮기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", 0.070).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "0.0"));
							moveIn3D ('y', plywood3.radAng, 0.070, &plywood3.posX, &plywood3.posY, &plywood3.posZ);
							moveIn3D ('z', plywood3.radAng, -moveZ, &plywood3.posX, &plywood3.posY, &plywood3.posZ);
				...
				// 여백에 다루끼(50*40) 배치
					...
					while (remainLengthInt > 0) {
						...
						elemList_Front.Push (timber.placeObject (6, "w_ins", APIParT_CString, "벽세우기", "w_w", APIParT_Length, format_string ("%f", 0.050).c_str(), "w_h", APIParT_Length, format_string ("%f", 0.040).c_str(), "w_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "w_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "torsion_ang", APIParT_Angle, format_string ("%f", 0.0).c_str()));
						moveIn3D ('x', timber.radAng, (double)lengthInt / 1000.0, &timber.posX, &timber.posY, &timber.posZ);
				...
				// 여백에 투바이(80*50) 배치
					...
					while (remainLengthInt > 0) {
						...
						elemList_Front.Push (timber.placeObject (6, "w_ins", APIParT_CString, "벽세우기", "w_w", APIParT_Length, format_string ("%f", 0.080).c_str(), "w_h", APIParT_Length, format_string ("%f", 0.050).c_str(), "w_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "w_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "torsion_ang", APIParT_Angle, format_string ("%f", 0.0).c_str()));
						moveIn3D ('x', timber.radAng, (double)lengthInt / 1000.0, &timber.posX, &timber.posY, &timber.posZ);
			...
			if (placingZone->bSingleSide == false) {
				...
				while (remainLengthInt > 0) {
					...
					if ((plywoodMarginExtra > EPS) && (lengthInt > 0)) {
						if (idxCell == 0) {
							if (bFirstPlywood == true)
								elemList_Back.Push (plywood.placeObject (13, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "벽눕히기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", plywoodMarginExtra).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "1.0", "bInverseSogak", APIParT_Boolean, "1.0", "prof", APIParT_CString, "소각", "gap_a", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_b", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_c", APIParT_Length, format_string ("%f", 0.070).c_str(), "gap_d", APIParT_Length, format_string ("%f", 0.0).c_str()));
							else
								elemList_Back.Push (plywood.placeObject (13, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "벽눕히기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", plywoodMarginExtra).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "1.0", "bInverseSogak", APIParT_Boolean, "1.0", "prof", APIParT_CString, "소각", "gap_a", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_b", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_c", APIParT_Length, format_string ("%f", 0.0).c_str(), "gap_d", APIParT_Length, format_string ("%f", 0.0).c_str()));
						} else if (idxCell == this->nCellsInHor-1) {
							if (bLastPlywood == true)
				...
				if (plywoodMarginExtra + EPS > 0.0) {
					...
					if ((plywoodMarginExtra >= 0.010 - EPS) && (plywoodMarginExtra <= 0.030 + EPS)) {
						...
						while (remainLengthInt > 0) {
							...
							elemList_Back.Push (timber.placeObject (6, "w_ins", APIParT_CString, "벽세우기", "w_w", APIParT_Length, format_string ("%f", 0.050).c_str(), "w_h", APIParT_Length, format_string ("%f", 0.080).c_str(), "w_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "w_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "torsion_ang", APIParT_Angle, format_string ("%f", 0.0).c_str()));
						...
						while (remainLengthInt > 0) {
							...
							if (addedPlywood >= 1) {
								...
								moveIn3D ('z', plywood1.radAng, moveZ, &plywood1.posX, &plywood1.posY, &plywood1.posZ);
								moveIn3D ('x', plywood1.radAng, -(double)lengthInt / 1000.0, &plywood1.posX, &plywood1.posY, &plywood1.posZ);
								elemList_Back.Push (plywood1.placeObject (7, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "바닥덮기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", 0.070).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "0.0"));
								moveIn3D ('y', plywood1.radAng, 0.070, &plywood1.posX, &plywood1.posY, &plywood1.posZ);
								moveIn3D ('z', plywood1.radAng, -moveZ, &plywood1.posX, &plywood1.posY, &plywood1.posZ);
							}
							if (addedPlywood >= 2) {
								...
								moveIn3D ('z', plywood2.radAng, moveZ, &plywood2.posX, &plywood2.posY, &plywood2.posZ);
								moveIn3D ('x', plywood2.radAng, -(double)lengthInt / 1000.0, &plywood2.posX, &plywood2.posY, &plywood2.posZ);
								elemList_Back.Push (plywood2.placeObject (7, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "바닥덮기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", 0.070).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "0.0"));
								moveIn3D ('y', plywood2.radAng, 0.070, &plywood2.posX, &plywood2.posY, &plywood2.posZ);
								moveIn3D ('z', plywood2.radAng, -moveZ, &plywood2.posX, &plywood2.posY, &plywood2.posZ);
							}
							if (addedPlywood >= 3) {
								...
								moveIn3D ('z', plywood3.radAng, moveZ, &plywood3.posX, &plywood3.posY, &plywood3.posZ);
								moveIn3D ('x', plywood3.radAng, -(double)lengthInt / 1000.0, &plywood3.posX, &plywood3.posY, &plywood3.posZ);
								elemList_Back.Push (plywood3.placeObject (7, "p_stan", APIParT_CString, "비규격", "w_dir", APIParT_CString, "바닥덮기", "p_thk", APIParT_CString, "11.5T", "p_wid", APIParT_Length, format_string ("%f", 0.070).c_str(), "p_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", 0.0).c_str(), "sogak", APIParT_Boolean, "0.0"));
								moveIn3D ('y', plywood3.radAng, 0.070, &plywood3.posX, &plywood3.posY, &plywood3.posZ);
								moveIn3D ('z', plywood3.radAng, -moveZ, &plywood3.posX, &plywood3.posY, &plywood3.posZ);
							}
					...
					// 여백에 다루끼(50*40) 배치
						...
						while (remainLengthInt > 0) {
							...
							moveIn3D ('x', timber.radAng - DegreeToRad (180.0), (double)lengthInt / 1000.0, &timber.posX, &timber.posY, &timber.posZ);
							elemList_Back.Push (timber.placeObject (6, "w_ins", APIParT_CString, "벽세우기", "w_w", APIParT_Length, format_string ("%f", 0.050).c_str(), "w_h", APIParT_Length, format_string ("%f", 0.040).c_str(), "w_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "w_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "torsion_ang", APIParT_Angle, format_string ("%f", 0.0).c_str()));
					...
					// 여백에 투바이(80*50) 배치
						...
						while (remainLengthInt > 0) {
							...
							moveIn3D ('x', timber.radAng - DegreeToRad (180.0), (double)lengthInt / 1000.0, &timber.posX, &timber.posY, &timber.posZ);
							elemList_Back.Push (timber.placeObject (6, "w_ins", APIParT_CString, "벽세우기", "w_w", APIParT_Length, format_string ("%f", 0.080).c_str(), "w_h", APIParT_Length, format_string ("%f", 0.050).c_str(), "w_leng", APIParT_Length, format_string ("%f", (double)lengthInt / 1000.0).c_str(), "w_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "torsion_ang", APIParT_Angle, format_string ("%f", 0.0).c_str()));
				...
				for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++yy) {
					...
					if (placingZone->cells [idxCell].tableInHor [yy] > 0) {
						...
						if (bStandard == true) {
							elemList_Front.Push (euroform.placeObject (5,
								"eu_stan_onoff", APIParT_Boolean, "1.0",
								"eu_wid", APIParT_CString, format_string ("%d", (int)(placingZone->marginCellsBasic [idxCell].formWidth1 * 1000)).c_str(),
								"eu_hei", APIParT_CString, format_string ("%d", placingZone->cells [idxCell].tableInHor [yy]).c_str(),
								"u_ins", APIParT_CString, "벽눕히기",
								"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
						} else {
							elemList_Front.Push (euroform.placeObject (5,
								"eu_stan_onoff", APIParT_Boolean, "0.0",
								"eu_wid2", APIParT_Length, format_string ("%f", placingZone->marginCellsBasic [idxCell].formWidth1).c_str(),
								"eu_hei2", APIParT_Length, format_string ("%f", (double)placingZone->cells [idxCell].tableInHor [yy] / 1000.0).c_str(),
								"u_ins", APIParT_CString, "벽눕히기",
								"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
						}
					}
				...
				if (placingZone->cells [idxCell].horLen > 0) {
					...
					if (bStandard == true) {
						elemList_Front.Push (euroform.placeObject (5,
							"eu_stan_onoff", APIParT_Boolean, "1.0",
							"eu_wid", APIParT_CString, format_string ("%d", (int)(placingZone->marginCellsBasic [idxCell].formWidth1 * 1000)).c_str(),
							"eu_hei", APIParT_CString, format_string ("%d", placingZone->cells [idxCell].horLen).c_str(),
							"u_ins", APIParT_CString, "벽눕히기",
							"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
					} else {
						elemList_Front.Push (euroform.placeObject (5,
							"eu_stan_onoff", APIParT_Boolean, "0.0",
							"eu_wid2", APIParT_Length, format_string ("%f", placingZone->marginCellsBasic [idxCell].formWidth1).c_str(),
							"eu_hei2", APIParT_Length, format_string ("%f", (double)placingZone->cells [idxCell].horLen / 1000.0).c_str(),
							"u_ins", APIParT_CString, "벽눕히기",
							"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
					}
				}
					...
					if (placingZone->cells [idxCell].tableInHor [yy] > 0) {
						...
						if (placingZone->cells [idxCell].tableInHor [yy] > 0) {
							if (bStandard == true) {
								elemList_Front.Push (euroform.placeObject (5,
									"eu_stan_onoff", APIParT_Boolean, "1.0",
									"eu_wid", APIParT_CString, format_string ("%d", (int)(placingZone->marginCellsBasic [idxCell].formWidth2 * 1000)).c_str(),
									"eu_hei", APIParT_CString, format_string ("%d", placingZone->cells [idxCell].tableInHor [yy]).c_str(),
									"u_ins", APIParT_CString, "벽눕히기",
									"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
							} else {
								elemList_Front.Push (euroform.placeObject (5,
									"eu_stan_onoff", APIParT_Boolean, "0.0",
									"eu_wid2", APIParT_Length, format_string ("%f", placingZone->marginCellsBasic [idxCell].formWidth2).c_str(),
									"eu_hei2", APIParT_Length, format_string ("%f", (double)placingZone->cells [idxCell].tableInHor [yy] / 1000.0).c_str(),
									"u_ins", APIParT_CString, "벽눕히기",
									"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
							}
					...
					for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++yy) {
						...
						if (placingZone->cells [idxCell].tableInHor [yy] > 0) {
							if (bStandard == true) {
								elemList_Back.Push (euroform.placeObject (5,
									"eu_stan_onoff", APIParT_Boolean, "1.0",
									"eu_wid", APIParT_CString, format_string ("%d", (int)(placingZone->marginCellsExtra [idxCell].formWidth1 * 1000)).c_str(),
									"eu_hei", APIParT_CString, format_string ("%d", placingZone->cells [idxCell].tableInHor [yy]).c_str(),
									"u_ins", APIParT_CString, "벽눕히기",
									"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
							} else {
								elemList_Back.Push (euroform.placeObject (5,
									"eu_stan_onoff", APIParT_Boolean, "0.0",
									"eu_wid2", APIParT_Length, format_string ("%f", placingZone->marginCellsExtra [idxCell].formWidth1).c_str(),
									"eu_hei2", APIParT_Length, format_string ("%f", (double)placingZone->cells [idxCell].tableInHor [yy] / 1000.0).c_str(),
									"u_ins", APIParT_CString, "벽눕히기",
									"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
							}
					...
					for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++yy) {
						...
						if (placingZone->cells [idxCell].tableInHor [yy] > 0) {
							if (bStandard == true) {
								elemList_Back.Push (euroform.placeObject (5,
									"eu_stan_onoff", APIParT_Boolean, "1.0",
									"eu_wid", APIParT_CString, format_string ("%d", (int)(placingZone->marginCellsExtra [idxCell].formWidth2 * 1000)).c_str(),
									"eu_hei", APIParT_CString, format_string ("%d", placingZone->cells [idxCell].tableInHor [yy]).c_str(),
									"u_ins", APIParT_CString, "벽눕히기",
									"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
							} else {
								elemList_Back.Push (euroform.placeObject (5,
									"eu_stan_onoff", APIParT_Boolean, "0.0",
									"eu_wid2", APIParT_Length, format_string ("%f", placingZone->marginCellsExtra [idxCell].formWidth2).c_str(),
									"eu_hei2", APIParT_Length, format_string ("%f", (double)placingZone->cells [idxCell].tableInHor [yy] / 1000.0).c_str(),
									"u_ins", APIParT_CString, "벽눕히기",
									"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
							}
				...
				for (yy = 0 ; yy < placingZone->nCellsInVerBasic ; ++yy) {
					if ((placingZone->cells [idxCell].tableInHor [xx] > 0) && (placingZone->cells [idxCell].tableInVerBasic [yy] > 0)) {
						elemList_Front.Push (euroform.placeObject (5,
							...
							"eu_hei", APIParT_CString, format_string ("%d", placingZone->cells [idxCell].tableInVerBasic [yy]).c_str(),
							"u_ins", APIParT_CString, "벽세우기",
							"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			...
			for (xx = 0 ; xx < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) ; ++xx) {
				...
				for (yy = 0 ; yy < placingZone->nCellsInVerBasic ; ++yy) {
					if ((placingZone->cells [idxCell].tableInHor [xx] > 0) && (placingZone->cells [idxCell].tableInVerBasic [yy] > 0)) {
						...
						elemList_Front.Push (euroform.placeObject (5,
							"eu_stan_onoff", APIParT_Boolean, "1.0",
							"eu_wid", APIParT_CString, format_string ("%d", placingZone->cells [idxCell].tableInVerBasic [yy]).c_str(),
...
void	WallTableformPlacingZone::placeTableformA (WallTableformPlacingZone* placingZone, short idxCell)
{
	...
	if (placingZone->bVertical == true) {
		...
		elemList_Front.Push (rectPipe.placeObject (4, "p_comp", APIParT_CString, "사각파이프", "p_leng", APIParT_Length, format_string ("%f", (double)pipeLength / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "bPunching", APIParT_Boolean, "0.0"));
		...
		if (placingZone->bSingleSide == false) {
			...
			moveIn3D ('z', rectPipe.radAng, -0.031, &rectPipe.posX, &rectPipe.posY, &rectPipe.posZ);	// 하부
			elemList_Back.Push (rectPipe.placeObject (4, "p_comp", APIParT_CString, "사각파이프", "p_leng", APIParT_Length, format_string ("%f", (double)pipeLength / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "bPunching", APIParT_Boolean, "0.0"));
			moveIn3D ('z', rectPipe.radAng, 0.062, &rectPipe.posX, &rectPipe.posY, &rectPipe.posZ);
			elemList_Back.Push (rectPipe.placeObject (4, "p_comp", APIParT_CString, "사각파이프", "p_leng", APIParT_Length, format_string ("%f", (double)pipeLength / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "bPunching", APIParT_Boolean, "0.0"));
			moveIn3D ('z', rectPipe.radAng, -0.031 - 0.450, &rectPipe.posX, &rectPipe.posY, &rectPipe.posZ);
		...
		elemList_Front.Push (rectPipe.placeObject (4, "p_comp", APIParT_CString, "사각파이프", "p_leng", APIParT_Length, format_string ("%f", (double)pipeLength / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(), "bPunching", APIParT_Boolean, "0.0"));
		...
		if (placingZone->cells [idxCell].horLen > 2300) {
			...
			elemList_Front_Add.Push (rectPipe.placeObject (4, "p_comp", APIParT_CString, "사각파이프", "p_leng", APIParT_Length, format_string ("%f", (double)pipeLength / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(), "bPunching", APIParT_Boolean, "0.0"));
			moveIn3D ('x', rectPipe.radAng, 0.070, &rectPipe.posX, &rectPipe.posY, &rectPipe.posZ);
			elemList_Front_Add.Push (rectPipe.placeObject (4, "p_comp", APIParT_CString, "사각파이프", "p_leng", APIParT_Length, format_string ("%f", (double)pipeLength / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(), "bPunching", APIParT_Boolean, "0.0"));
		}
		...
		if (placingZone->bSingleSide == false) {
			...
			moveIn3D ('x', rectPipe.radAng, -0.035, &rectPipe.posX, &rectPipe.posY, &rectPipe.posZ);	// 왼쪽
			elemList_Back.Push (rectPipe.placeObject (4, "p_comp", APIParT_CString, "사각파이프", "p_leng", APIParT_Length, format_string ("%f", (double)pipeLength / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(), "bPunching", APIParT_Boolean, "0.0"));
			moveIn3D ('x', rectPipe.radAng, 0.070, &rectPipe.posX, &rectPipe.posY, &rectPipe.posZ);
			elemList_Back.Push (rectPipe.placeObject (4, "p_comp", APIParT_CString, "사각파이프", "p_leng", APIParT_Length, format_string ("%f", (double)pipeLength / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(), "bPunching", APIParT_Boolean, "0.0"));
			...
			if (placingZone->cells [idxCell].horLen > 2300) {
				...
				elemList_Back_Add.Push (rectPipe.placeObject (4, "p_comp", APIParT_CString, "사각파이프", "p_leng", APIParT_Length, format_string ("%f", (double)pipeLength / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(), "bPunching", APIParT_Boolean, "0.0"));
							...
							if (count == realWidthCount) {
								...
								if (placingZone->cells [idxCell].tableInHor [yy] == 600) {
									moveIn3D ('x', pinbolt.radAng, 0.150, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
										elemList_Front.Push (pinbolt.placeObject (7, "bRotated", APIParT_Boolean, "0.0", "bolt_len", APIParT_Length, format_string ("%f", 0.100).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.010).c_str(), "washer_pos", APIParT_Length, format_string ("%f", 0.050).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (270.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
									moveIn3D ('x', pinbolt.radAng, 0.300, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
										...
										elemList_Front.Push (pinbolt.placeObject (7, "bRotated", APIParT_Boolean, "0.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.010).c_str(), "washer_pos", APIParT_Length, format_string ("%f", 0.050).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (270.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
										moveIn3D ('y', pinbolt.radAng, 0.050, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
									moveIn3D ('x', pinbolt.radAng, 0.150, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
								} else if (placingZone->cells [idxCell].tableInHor [yy] == 500) {
									moveIn3D ('x', pinbolt.radAng, 0.150, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
									...
									moveIn3D ('x', pinbolt.radAng, 0.050, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
										moveIn3D ('y', pinbolt.radAng, -0.050, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
										elemList_Front.Push (pinbolt.placeObject (7, "bRotated", APIParT_Boolean, "0.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.010).c_str(), "washer_pos", APIParT_Length, format_string ("%f", 0.050).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (270.0)), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0))));
										moveIn3D ('y', pinbolt.radAng, 0.050, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
									moveIn3D ('x', pinbolt.radAng, 0.150, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
								}
							} else if (count == 1) {
		...
		if (placingZone->cells [idxCell].horLen > 2300) {
			...
			for (xx = 0 ; xx < placingZone->nCellsInVerBasic - 1 ; ++xx) {
				if (placingZone->cells [idxCell].tableInVerBasic [xx] > 0) {
					moveIn3D ('z', pinbolt.radAng, (double)placingZone->cells [idxCell].tableInVerBasic [xx] / 1000.0, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
					elemList_Front_Add.Push (pinbolt.placeObject (7, "bRotated", APIParT_Boolean, "0.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.010).c_str(), "washer_pos", APIParT_Length, format_string ("%f", 0.050).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (270.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
				}
			}
		}
		...
		if (placingZone->bSingleSide == false) {
			...
			for (xx = 0 ; xx < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) - 1 ; ++xx) {	// 하부
				if (placingZone->cells [idxCell].tableInHor [xx] > 0) {
					...
					if (count > 0) {
						pinbolt.radAng += DegreeToRad (90.0);
						elemList_Back.Push (pinbolt.placeObject (7, "bRotated", APIParT_Boolean, "0.0", "bolt_len", APIParT_Length, format_string ("%f", 0.100).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.010).c_str(), "washer_pos", APIParT_Length, format_string ("%f", 0.050).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
						pinbolt.radAng -= DegreeToRad (90.0);
								...
								if (count == realWidthCount) {
									...
									if (placingZone->cells [idxCell].tableInHor [yy] == 600) {
										...
										moveIn3D ('x', pinbolt.radAng - DegreeToRad (180.0), 0.300, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
											...
											elemList_Back.Push (pinbolt.placeObject (7, "bRotated", APIParT_Boolean, "0.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.010).c_str(), "washer_pos", APIParT_Length, format_string ("%f", 0.050).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (270.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
											moveIn3D ('y', pinbolt.radAng - DegreeToRad (180.0), -0.050, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
										moveIn3D ('x', pinbolt.radAng - DegreeToRad (180.0), 0.150, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
									} else if (placingZone->cells [idxCell].tableInHor [yy] == 500) {
										moveIn3D ('x', pinbolt.radAng - DegreeToRad (180.0), 0.150, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
		...
		elemList_Front.Push (join.placeObject (11, "bRotated", APIParT_Boolean, "0.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.012).c_str(), "bWasher1", APIParT_Boolean, "1.0", "washer_pos1", APIParT_Length, format_string ("%f", 0.000).c_str(), "bWasher2", APIParT_Boolean, "1.0", "washer_pos2", APIParT_Length, format_string ("%f", 0.108).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "nutType", APIParT_CString, "육각너트", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		...
		if (placingZone->cells [idxCell].horLen > 2300) {
			...
			elemList_Front_Add.Push (join.placeObject (11, "bRotated", APIParT_Boolean, "0.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.012).c_str(), "bWasher1", APIParT_Boolean, "1.0", "washer_pos1", APIParT_Length, format_string ("%f", 0.000).c_str(), "bWasher2", APIParT_Boolean, "1.0", "washer_pos2", APIParT_Length, format_string ("%f", 0.108).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "nutType", APIParT_CString, "육각너트", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		...
		if (placingZone->bSingleSide == false) {
			...
			elemList_Back.Push (join.placeObject (11, "bRotated", APIParT_Boolean, "0.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.012).c_str(), "bWasher1", APIParT_Boolean, "1.0", "washer_pos1", APIParT_Length, format_string ("%f", 0.000).c_str(), "bWasher2", APIParT_Boolean, "1.0", "washer_pos2", APIParT_Length, format_string ("%f", 0.108).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "nutType", APIParT_CString, "육각너트", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
			moveIn3D ('x', join.radAng, -(double)(150 - firstWidth + placingZone->cells [idxCell].horLen - lastWidth + 150) / 1000.0, &join.posX, &join.posY, &join.posZ);
			elemList_Back.Push (join.placeObject (11, "bRotated", APIParT_Boolean, "0.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.012).c_str(), "bWasher1", APIParT_Boolean, "1.0", "washer_pos1", APIParT_Length, format_string ("%f", 0.000).c_str(), "bWasher2", APIParT_Boolean, "1.0", "washer_pos2", APIParT_Length, format_string ("%f", 0.108).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "nutType", APIParT_CString, "육각너트", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		...
		elemList_Front.Push (headpiece.placeObject (4, "type", APIParT_CString, "타입 A", "plateThk", APIParT_Length, format_string ("%f", 0.009).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		...
		if (placingZone->cells [idxCell].horLen > 2300) {
			...
			elemList_Front_Add.Push (headpiece.placeObject (4, "type", APIParT_CString, "타입 A", "plateThk", APIParT_Length, format_string ("%f", 0.009).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		...
		if ((placingZone->propsInstallType == 2) || (placingZone->propsInstallType == 4)) {
			...
			moveIn3D ('y', props.radAng, -(0.2525 + 0.080 + 1.200), &props.posX, &props.posY, &props.posZ);
				...
				elemList_Front.Push (props.placeObject (14, "bBasePlate", APIParT_Boolean, "1.0", "typeBasePlate", APIParT_CString, "당사 제작품", "angX", APIParT_Angle, "0.0", "angY", APIParT_Angle, "0.0", "mainType", APIParT_CString, mainNom, "bMainQuickConnect", APIParT_Boolean, "0.0", "mainLength", APIParT_Length, format_string ("%f", mainLength).c_str(), "mainAng", APIParT_Angle, format_string ("%f", mainRadAng).c_str(), "bAuxSupport", APIParT_Boolean, "1.0", "auxType", APIParT_CString, auxNom, "bAuxQuickConnect", APIParT_Boolean, "0.0", "auxLength", APIParT_Length, format_string ("%f", auxLength).c_str(), "auxAng", APIParT_Angle, format_string ("%f", auxRadAng).c_str(), "bShowHotspotsOnBase", APIParT_Boolean, "0.0"));
				props.radAng -= DegreeToRad (90.0);
		...
		if (placingZone->bSingleSide == false) {
			...
			elemList_Back.Push (headpiece.placeObject (4, "type", APIParT_CString, "타입 A", "plateThk", APIParT_Length, format_string ("%f", 0.009).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
			...
			if (placingZone->cells [idxCell].horLen > 2300) {
				...
				elemList_Back_Add.Push (headpiece.placeObject (4, "type", APIParT_CString, "타입 A", "plateThk", APIParT_Length, format_string ("%f", 0.009).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		...
		for (xx = 0 ; xx < sizeof (placingZone->cells [idxCell].tableInVerBasic) / sizeof (int) - 1 ; ++xx) {	// 하부
			if (placingZone->cells [idxCell].tableInVerBasic [xx] > 0) {
				...
				if (count > 0) {
					pinbolt.radAng += DegreeToRad (90.0);
					elemList_Front.Push (pinbolt.placeObject (7, "bRotated", APIParT_Boolean, "1.0", "bolt_len", APIParT_Length, format_string ("%f", 0.100).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.010).c_str(), "washer_pos", APIParT_Length, format_string ("%f", 0.050).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
					pinbolt.radAng -= DegreeToRad (90.0);
							...
							if (count == realHeightCount) {
								...
								if (placingZone->cells [idxCell].tableInVerBasic [yy] == 600) {
									...
									moveIn3D ('z', pinbolt.radAng, 0.300, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
										moveIn3D ('y', pinbolt.radAng, -0.050, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
										elemList_Front.Push (pinbolt.placeObject (7, "bRotated", APIParT_Boolean, "1.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.010).c_str(), "washer_pos", APIParT_Length, format_string ("%f", 0.050).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (270.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
										moveIn3D ('y', pinbolt.radAng, 0.050, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
									moveIn3D ('z', pinbolt.radAng, 0.150, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
		...
		if (placingZone->bSingleSide == false) {
			...
			for (xx = 0 ; xx < sizeof (placingZone->cells [idxCell].tableInVerBasic) / sizeof (int) - 1 ; ++xx) {	// 하부
				if (intPointer [xx] > 0) {
					...
					if (count > 0) {
						pinbolt.radAng += DegreeToRad (90.0);
						elemList_Back.Push (pinbolt.placeObject (7, "bRotated", APIParT_Boolean, "1.0", "bolt_len", APIParT_Length, format_string ("%f", 0.100).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.010).c_str(), "washer_pos", APIParT_Length, format_string ("%f", 0.050).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
						pinbolt.radAng -= DegreeToRad (90.0);
								...
								if (count == realHeightCount) {
									...
									if (intPointer [yy] == 600) {
										...
										moveIn3D ('z', pinbolt.radAng - DegreeToRad (180.0), 0.300, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
											moveIn3D ('y', pinbolt.radAng - DegreeToRad (180.0), 0.050, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
											elemList_Back.Push (pinbolt.placeObject (7, "bRotated", APIParT_Boolean, "1.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.010).c_str(), "washer_pos", APIParT_Length, format_string ("%f", 0.050).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (270.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
											moveIn3D ('y', pinbolt.radAng - DegreeToRad (180.0), -0.050, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
										moveIn3D ('z', pinbolt.radAng - DegreeToRad (180.0), 0.150, &pinbolt.posX, &pinbolt.posY, &pinbolt.posZ);
		...
		elemList_Front.Push (join.placeObject (11, "bRotated", APIParT_Boolean, "1.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.012).c_str(), "bWasher1", APIParT_Boolean, "1.0", "washer_pos1", APIParT_Length, format_string ("%f", 0.000).c_str(), "bWasher2", APIParT_Boolean, "1.0", "washer_pos2", APIParT_Length, format_string ("%f", 0.108).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "nutType", APIParT_CString, "육각너트", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		...
		if (placingZone->bSingleSide == false) {
			...
			elemList_Back.Push (join.placeObject (11, "bRotated", APIParT_Boolean, "1.0", "bolt_len", APIParT_Length, format_string ("%f", 0.150).c_str(), "bolt_dia", APIParT_Length, format_string ("%f", 0.012).c_str(), "bWasher1", APIParT_Boolean, "1.0", "washer_pos1", APIParT_Length, format_string ("%f", 0.000).c_str(), "bWasher2", APIParT_Boolean, "1.0", "washer_pos2", APIParT_Length, format_string ("%f", 0.108).c_str(), "washer_size", APIParT_Length, format_string ("%f", 0.100).c_str(), "nutType", APIParT_CString, "육각너트", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		...
		elemList_Front.Push (headpiece.placeObject (4, "type", APIParT_CString, "타입 B", "plateThk", APIParT_Length, format_string ("%f", 0.009).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
		...
		if (placingZone->bSingleSide == false) {
			...
			elemList_Back.Push (headpiece.placeObject (4, "type", APIParT_CString, "타입 B", "plateThk", APIParT_Length, format_string ("%f", 0.009).c_str(), "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
...
void	WallTableformPlacingZone::placeTableformB (WallTableformPlacingZone* placingZone, short idxCell)
{
	...
	if (placingZone->bVertical == true) {
		...
		elemList_Front.Push (rectPipe.placeObject (7, "p_comp", APIParT_CString, "사각파이프", "p_leng", APIParT_Length, format_string ("%f", (double)pipeLength / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "bPunching", APIParT_Boolean, "1.0", "holeDir", APIParT_CString, "정면", "holeDia", APIParT_Length, "0.013", "holeDist", APIParT_Length, "0.050"));
		moveIn3D ('z', rectPipe.radAng, -0.450, &rectPipe.posX, &rectPipe.posY, &rectPipe.posZ);
		...
		moveIn3D ('z', rectPipe.radAng, (double)placingZone->cells [idxCell].tableInVerBasic [placingZone->nCellsInVerBasic - 1] / 1000.0 - 0.150, &rectPipe.posX, &rectPipe.posY, &rectPipe.posZ);		// 상부
		...
		if (placingZone->bSingleSide == false) {
			...
			moveIn3D ('z', rectPipe.radAng, (double)intPointer [varEnd - 1] / 1000.0 - 0.150, &rectPipe.posX, &rectPipe.posY, &rectPipe.posZ);		// 상부
		...
		elemList_Front.Push (rectPipe.placeObject (7, "p_comp", APIParT_CString, "사각파이프", "p_leng", APIParT_Length, format_string ("%f", (double)pipeLength / 1000.0).c_str(), "p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(), "bPunching", APIParT_Boolean, "1.0", "holeDir", APIParT_CString, "정면", "holeDia", APIParT_Length, "0.013", "holeDist", APIParT_Length, "0.050"));
		...
		elemList_Front.Push (headpiece.placeObject (3, "type", APIParT_CString, "타입 A", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		...
		if (placingZone->bSingleSide == false) {
			...
			elemList_Back.Push (headpiece.placeObject (3, "type", APIParT_CString, "타입 A", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		...
		elemList_Front.Push (headpiece.placeObject (3, "type", APIParT_CString, "타입 B", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		moveIn3D ('z', headpiece.radAng, (double)(-(bottomHeight - 150) + placingZone->cells [idxCell].verLenBasic + (-topHeight + 150)) / 1000.0, &headpiece.posX, &headpiece.posY, &headpiece.posZ);
		elemList_Front.Push (headpiece.placeObject (3, "type", APIParT_CString, "타입 B", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		...
		if (placingZone->bSingleSide == false) {
			...
			elemList_Back.Push (headpiece.placeObject (3, "type", APIParT_CString, "타입 B", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
			moveIn3D ('z', headpiece.radAng - DegreeToRad (180.0), (double)(-(bottomHeight - 150) + backHeight + (-topHeight + 150)) / 1000.0, &headpiece.posX, &headpiece.posY, &headpiece.posZ);
			elemList_Back.Push (headpiece.placeObject (3, "type", APIParT_CString, "타입 B", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
			...
			elemList_Back.Push (headpiece.placeObject (4, "type", APIParT_CString, "타입 B", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
			moveIn3D ('z', headpiece.radAng - DegreeToRad (180.0), (double)(-(bottomHeight - 150) + backHeight + (-topHeight + 150)) / 1000.0, &headpiece.posX, &headpiece.posY, &headpiece.posZ);
			elemList_Back.Push (headpiece.placeObject (4, "type", APIParT_CString, "타입 B", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
		}
	}
		...
		for (xx = 0 ; xx < varEnd - 1 ; ++xx) {
			...
			for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) - 1 ; ++yy) {
				if (placingZone->cells [idxCell].tableInHor [yy] > 0 && count > 1) {
					...
					elemList_Front.Push (cross.placeObject (3, "bRotated", APIParT_Boolean, "1.0", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
					count --;
				}
			}
			...
			for (xx = 0 ; xx < varEnd - 1 ; ++xx) {
				...
				for (yy = 0 ; yy < sizeof (placingZone->cells [idxCell].tableInHor) / sizeof (int) - 1 ; ++yy) {
					if (placingZone->cells [idxCell].tableInHor [yy] > 0 && count > 1) {
						...
						elemList_Back.Push (cross.placeObject (3, "bRotated", APIParT_Boolean, "1.0", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
						count --;
					}
				}
...
short DGCALLBACK wallTableformPlacerHandler1 (short message, short dialogID, short item, DGUserData /* userData */, DGMessageData /* msgData */)
{
	...
	switch (message) {
		case DG_MSG_INIT:
			...
			DGSetItemText (dialogID, itmIdx, L"정면");
			...
			DGSetItemFont (dialogID, itmIdx, DG_IS_LARGE | DG_IS_PLAIN);
			DGSetItemText (dialogID, itmIdx, L"벽과의 간격");
			DGShowItem (dialogID, itmIdx);
			...
			DGSetItemFont (dialogID, itmIdx, DG_IS_LARGE | DG_IS_PLAIN);
			DGSetItemText (dialogID, itmIdx, L"양면/단면");
			DGShowItem (dialogID, itmIdx);
			...
			DGSetItemText (dialogID, placingZone.CHECKBOX_SINGLESIDE, L"단면");
			...
			DGPopUpInsertItem (dialogID, placingZone.POPUP_TABLEFORM_TYPE, DG_POPUP_BOTTOM);
			DGPopUpSetItemText (dialogID, placingZone.POPUP_TABLEFORM_TYPE, DG_POPUP_BOTTOM, L"타입B");
			DGPopUpInsertItem (dialogID, placingZone.POPUP_TABLEFORM_TYPE, DG_POPUP_BOTTOM);
			DGPopUpSetItemText (dialogID, placingZone.POPUP_TABLEFORM_TYPE, DG_POPUP_BOTTOM, L"타입C");
			...
			DGSetItemFont (dialogID, itmIdx, DG_IS_LARGE | DG_IS_PLAIN);
			DGSetItemText (dialogID, itmIdx, L"푸시풀프롭스");
			DGShowItem (dialogID, itmIdx);
			...
			DGPopUpSetItemText (dialogID, placingZone.POPUP_PROPS_INSTALL, DG_POPUP_BOTTOM, L"없음");
			DGPopUpInsertItem (dialogID, placingZone.POPUP_PROPS_INSTALL, DG_POPUP_BOTTOM);
			DGPopUpSetItemText (dialogID, placingZone.POPUP_PROPS_INSTALL, DG_POPUP_BOTTOM, L"안쪽");
			DGPopUpInsertItem (dialogID, placingZone.POPUP_PROPS_INSTALL, DG_POPUP_BOTTOM);
			DGPopUpSetItemText (dialogID, placingZone.POPUP_PROPS_INSTALL, DG_POPUP_BOTTOM, L"바깥쪽");
			DGPopUpInsertItem (dialogID, placingZone.POPUP_PROPS_INSTALL, DG_POPUP_BOTTOM);
			DGPopUpSetItemText (dialogID, placingZone.POPUP_PROPS_INSTALL, DG_POPUP_BOTTOM, L"양쪽");
			...
			DGSetItemFont (dialogID, itmIdx, DG_IS_LARGE | DG_IS_BOLD);
			DGSetItemText (dialogID, itmIdx, L"측면");
			DGShowItem (dialogID, itmIdx);
			...
			DGSetItemText (dialogID, placingZone.BUTTON_ADD_VER_BASIC, L"추가(L)");
			...
			DGSetItemText (dialogID, placingZone.BUTTON_DEL_VER_BASIC, L"삭제(L)");
			...
			DGSetItemFont (dialogID, itmIdx, DG_IS_LARGE | DG_IS_PLAIN);
			DGSetItemText (dialogID, itmIdx, L"남은 높이");
			DGShowItem (dialogID, itmIdx);
			...
			if (placingZone.bExtra == true) {
				...
				DGSetItemText (dialogID, placingZone.BUTTON_ADD_VER_EXTRA, L"추가(H)");
				...
				DGSetItemText (dialogID, placingZone.BUTTON_DEL_VER_EXTRA, L"삭제(H)");
			...
			DGPopUpInsertItem (dialogID, placingZone.POPUP_HEIGHT_PRESET, DG_POPUP_BOTTOM);
			DGPopUpSetItemText (dialogID, placingZone.POPUP_HEIGHT_PRESET, DG_POPUP_BOTTOM, L"Free");
		...
		case DG_MSG_CLICK:
			...
			if (item == DG_OK) {
				...
				// 푸시풀프롭스 설치 타입
				if (DGPopUpGetSelected (dialogID, placingZone.POPUP_PROPS_INSTALL) == PUSHPULLPROPS_NONE)
					placingZone.propsInstallType = 1;
				else if (DGPopUpGetSelected (dialogID, placingZone.POPUP_PROPS_INSTALL) == PUSHPULLPROPS_INNER)
					placingZone.propsInstallType = 2;
				else if (DGPopUpGetSelected (dialogID, placingZone.POPUP_PROPS_INSTALL) == PUSHPULLPROPS_OUTER)
					placingZone.propsInstallType = 3;
				else if (DGPopUpGetSelected (dialogID, placingZone.POPUP_PROPS_INSTALL) == PUSHPULLPROPS_INOUT)
...
short DGCALLBACK wallTableformPlacerHandler4 (short message, short dialogID, short item, DGUserData /* userData */, DGMessageData /* msgData */)
{
	...
	switch (message) {
		case DG_MSG_INIT:
			// 다이얼로그 타이틀
			if (iMarginSide == 1)
				DGSetDialogTitle (dialogID, L"벽 상단 채우기 (낮은쪽)");
			else
				DGSetDialogTitle (dialogID, L"벽 상단 채우기 (높은쪽)");
			...
			DGSetItemText (dialogID, LABEL_DESC1_TOPREST, L"벽 상부에 다음 높이 만큼의 공간이 있습니다.");
			...
			DGSetItemText (dialogID, LABEL_HEIGHT_TOPREST, L"높이");
			...
			DGSetItemText (dialogID, LABEL_DESC2_TOPREST, L"유로폼으로 채우시겠습니까?");
			...
			DGSetItemText (dialogID, LABEL_UP_TOPREST, L"위");
			...
			DGSetItemText (dialogID, LABEL_ARROWUP_TOPREST, L"↑");
			...
			DGSetItemText (dialogID, LABEL_DOWN_TOPREST, L"아래");
			...
			DGSetItemText (dialogID, CHECKBOX_FORM_ONOFF_1_TOPREST, L"유로폼");
			...
			DGSetItemText (dialogID, CHECKBOX_FORM_ONOFF_2_TOPREST, L"유로폼");
			...
			DGSetItemText (dialogID, LABEL_PLYWOOD_TOPREST, L"합판");
			...
			DGSetItemText (dialogID, CHECKBOX_SET_STANDARD_1_TOPREST, L"규격폼");
			...
			DGSetItemText (dialogID, CHECKBOX_SET_STANDARD_2_TOPREST, L"규격폼");
			...
			if (iMarginSide == 1) {
				if (placingZone.marginTopBasic < EPS) {
					DGSetItemText (dialogID, LABEL_PLYWOOD_TOPREST, L"없음");
				} else if (placingZone.marginTopBasic <= 0.100 + EPS) {
					DGSetItemText (dialogID, LABEL_PLYWOOD_TOPREST, L"목재");
				} else {
					DGSetItemText (dialogID, LABEL_PLYWOOD_TOPREST, L"합판");
				}
