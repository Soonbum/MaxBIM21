...
GSErrCode	placeTableformOnColumn (void)
{
	...
	placingZone.workLevel = workLevel_column;
	placingZone.baseElev = infoColumn.bottomOffset + placingZone.workLevel;
	...
	for (xx = 0 ; xx < nInterfereBeams ; ++xx) {
		...
		infoOtherBeams [xx].workLevel = workLevel_beam;
		infoOtherBeams [xx].baseElev = infoOtherBeams [xx].level - infoOtherBeams [xx].height + infoOtherBeams [xx].workLevel;
	}
	...
	lowestBeamBottomLevel = placingZone.baseElev + placingZone.areaHeight;	// 기본값: 영역 높이
	if (nInterfereBeams > 0) {
	...
	placingZone.nCells = static_cast<short>(((lowestBeamBottomLevel - placingZone.baseElev) + EPS) / 1.200);		// 높이 1200 폼 기준으로 들어갈 수 있는 최대 셀 개수 연산
...
GSErrCode	ColumnTableformPlacingZone::placeBasicObjects_soleColumn (ColumnTableformPlacingZone* placingZone)
{
	...
	if ((placingZone->cellsT2 [0].horLen > EPS) && (placingZone->cellsT2 [0].height > EPS)) {
		...
		while (remainLength > EPS) {
			...
			elemList.Push (fillersp.placeObject (4,
				"f_thk", APIParT_Length, format_string ("%f", placingZone->cellsT2 [0].horLen).c_str(),
				"f_leng", APIParT_Length, format_string ("%f", length).c_str(),
	...
	if ((placingZone->cellsB2 [0].horLen > EPS) && (placingZone->cellsB2 [0].height > EPS)) {
		...
		while (remainLength > EPS) {
			...
			elemList.Push (fillersp.placeObject (4,
				"f_thk", APIParT_Length, format_string ("%f", placingZone->cellsB2 [0].horLen).c_str(),
				"f_leng", APIParT_Length, format_string ("%f", length).c_str(),
				"f_ang", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
				"f_rota", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
			moveIn3D ('z', fillersp.radAng, length, &fillersp.posX, &fillersp.posY, &fillersp.posZ);
	...
	if ((placingZone->cellsL2 [0].verLen > EPS) && (placingZone->cellsL2 [0].height > EPS)) {
		...
		while (remainLength > EPS) {
			...
			elemList.Push (fillersp.placeObject (4,
				"f_thk", APIParT_Length, format_string ("%f", placingZone->cellsL2 [0].verLen).c_str(),
				"f_leng", APIParT_Length, format_string ("%f", length).c_str(),
				"f_ang", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
				"f_rota", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
			moveIn3D ('z', fillersp.radAng, length, &fillersp.posX, &fillersp.posY, &fillersp.posZ);
	...
	if ((placingZone->cellsR2 [0].verLen > EPS) && (placingZone->cellsR2 [0].height > EPS)) {
		...
		while (remainLength > EPS) {
			...
			elemList.Push (fillersp.placeObject (4,
				"f_thk", APIParT_Length, format_string ("%f", placingZone->cellsR2 [0].verLen).c_str(),
				"f_leng", APIParT_Length, format_string ("%f", length).c_str(),
				"f_ang", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
				"f_rota", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str()));
			moveIn3D ('z', fillersp.radAng, length, &fillersp.posX, &fillersp.posY, &fillersp.posZ);
	...
	// 아웃코너앵글 배치
	// 좌상단
	outangle.init (L("아웃코너앵글v1.0.gsm"), layerInd_OutAngle, infoColumn.floorInd, placingZone->cellsLT [0].leftBottomX, placingZone->cellsLT [0].leftBottomY, placingZone->cellsLT [0].leftBottomZ, placingZone->cellsLT [0].ang);
	if ((placingZone->cellsLT [0].objType == OUTANGLE) && (placingZone->cellsLT [0].height > EPS)) {
		...
		while (remainLength > EPS) {
			...
			elemList.Push (outangle.placeObject (2,
				"a_leng", APIParT_Length, format_string ("%f", length).c_str(),
	...
	// 우상단
	outangle.init (L("아웃코너앵글v1.0.gsm"), layerInd_OutAngle, infoColumn.floorInd, placingZone->cellsRT [0].leftBottomX, placingZone->cellsRT [0].leftBottomY, placingZone->cellsRT [0].leftBottomZ, placingZone->cellsRT [0].ang);
	...
	// 좌하단
	outangle.init (L("아웃코너앵글v1.0.gsm"), layerInd_OutAngle, infoColumn.floorInd, placingZone->cellsLB [0].leftBottomX, placingZone->cellsLB [0].leftBottomY, placingZone->cellsLB [0].leftBottomZ, placingZone->cellsLB [0].ang);
	...
	// 우하단
	outangle.init (L("아웃코너앵글v1.0.gsm"), layerInd_OutAngle, infoColumn.floorInd, placingZone->cellsRB [0].leftBottomX, placingZone->cellsRB [0].leftBottomY, placingZone->cellsRB [0].leftBottomZ, placingZone->cellsRB [0].ang);
	...
	for (xx = 0 ; xx < placingZone->nCells ; ++xx) {
		...
		if ((placingZone->cellsT1 [xx].horLen > EPS) && (placingZone->cellsT1 [xx].height > EPS)) {
			if (placingZone->cellsT1 [xx].bStandardEuroform == true) {
				elemList.Push (euroform.placeObject (6,
					...
					"eu_wid", APIParT_CString, format_string ("%.0f", placingZone->cellsT1 [xx].horLen * 1000).c_str(),
					"eu_hei", APIParT_CString, format_string ("%.0f", placingZone->cellsT1 [xx].height * 1000).c_str(),
				...
				elemList.Push (euroform.placeObject (6,
					...
					"eu_wid2", APIParT_Length, format_string ("%f", placingZone->cellsT1 [xx].horLen).c_str(),
					"eu_hei2", APIParT_Length, format_string ("%f", placingZone->cellsT1 [xx].height).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			}
		...
		if ((placingZone->cellsT3 [xx].horLen > EPS) && (placingZone->cellsT3 [xx].height > EPS)) {
			if (placingZone->cellsT3 [xx].bStandardEuroform == true) {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "1.0",
					"eu_wid", APIParT_CString, format_string ("%.0f", placingZone->cellsT3 [xx].horLen * 1000).c_str(),
					"eu_hei", APIParT_CString, format_string ("%.0f", placingZone->cellsT3 [xx].height * 1000).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			} else {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "0.0",
					"eu_wid2", APIParT_Length, format_string ("%f", placingZone->cellsT3 [xx].horLen).c_str(),
					"eu_hei2", APIParT_Length, format_string ("%f", placingZone->cellsT3 [xx].height).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			}
		...
		if ((placingZone->cellsB1 [xx].horLen > EPS) && (placingZone->cellsB1 [xx].height > EPS)) {
			if (placingZone->cellsB1 [xx].bStandardEuroform == true) {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "1.0",
					"eu_wid", APIParT_CString, format_string ("%.0f", placingZone->cellsB1 [xx].horLen * 1000).c_str(),
					"eu_hei", APIParT_CString, format_string ("%.0f", placingZone->cellsB1 [xx].height * 1000).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			} else {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "0.0",
					"eu_wid2", APIParT_Length, format_string ("%f", placingZone->cellsB1 [xx].horLen).c_str(),
					"eu_hei2", APIParT_Length, format_string ("%f", placingZone->cellsB1 [xx].height).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			}
		...
		if ((placingZone->cellsB3 [xx].horLen > EPS) && (placingZone->cellsB3 [xx].height > EPS)) {
			if (placingZone->cellsB3 [xx].bStandardEuroform == true) {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "1.0",
					"eu_wid", APIParT_CString, format_string ("%.0f", placingZone->cellsB3 [xx].horLen * 1000).c_str(),
					"eu_hei", APIParT_CString, format_string ("%.0f", placingZone->cellsB3 [xx].height * 1000).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			} else {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "0.0",
					"eu_wid2", APIParT_Length, format_string ("%f", placingZone->cellsB3 [xx].horLen).c_str(),
					"eu_hei2", APIParT_Length, format_string ("%f", placingZone->cellsB3 [xx].height).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			}
		...
		if ((placingZone->cellsL1 [xx].verLen > EPS) && (placingZone->cellsL1 [xx].height > EPS)) {
			if (placingZone->cellsL1 [xx].bStandardEuroform == true) {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "1.0",
					"eu_wid", APIParT_CString, format_string ("%.0f", placingZone->cellsL1 [xx].verLen * 1000).c_str(),
					"eu_hei", APIParT_CString, format_string ("%.0f", placingZone->cellsL1 [xx].height * 1000).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			} else {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "0.0",
					"eu_wid2", APIParT_Length, format_string ("%f", placingZone->cellsL1 [xx].verLen).c_str(),
					"eu_hei2", APIParT_Length, format_string ("%f", placingZone->cellsL1 [xx].height).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			}
		...
		if ((placingZone->cellsL3 [xx].verLen > EPS) && (placingZone->cellsL3 [xx].height > EPS)) {
			if (placingZone->cellsL3 [xx].bStandardEuroform == true) {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "1.0",
					"eu_wid", APIParT_CString, format_string ("%.0f", placingZone->cellsL3 [xx].verLen * 1000).c_str(),
					"eu_hei", APIParT_CString, format_string ("%.0f", placingZone->cellsL3 [xx].height * 1000).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			} else {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "0.0",
					"eu_wid2", APIParT_Length, format_string ("%f", placingZone->cellsL3 [xx].verLen).c_str(),
					"eu_hei2", APIParT_Length, format_string ("%f", placingZone->cellsL3 [xx].height).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			}
		...
		if ((placingZone->cellsR1 [xx].verLen > EPS) && (placingZone->cellsR1 [xx].height > EPS)) {
			if (placingZone->cellsR1 [xx].bStandardEuroform == true) {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "1.0",
					"eu_wid", APIParT_CString, format_string ("%.0f", placingZone->cellsR1 [xx].verLen * 1000).c_str(),
					"eu_hei", APIParT_CString, format_string ("%.0f", placingZone->cellsR1 [xx].height * 1000).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			} else {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "0.0",
					"eu_wid2", APIParT_Length, format_string ("%f", placingZone->cellsR1 [xx].verLen).c_str(),
					"eu_hei2", APIParT_Length, format_string ("%f", placingZone->cellsR1 [xx].height).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			}
		...
		if ((placingZone->cellsR3 [xx].verLen > EPS) && (placingZone->cellsR3 [xx].height > EPS)) {
			if (placingZone->cellsR3 [xx].bStandardEuroform == true) {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "1.0",
					"eu_wid", APIParT_CString, format_string ("%.0f", placingZone->cellsR3 [xx].verLen * 1000).c_str(),
					"eu_hei", APIParT_CString, format_string ("%.0f", placingZone->cellsR3 [xx].height * 1000).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			} else {
				elemList.Push (euroform.placeObject (6,
					"eu_stan_onoff", APIParT_Boolean, "0.0",
					"eu_wid2", APIParT_Length, format_string ("%f", placingZone->cellsR3 [xx].verLen).c_str(),
					"eu_hei2", APIParT_Length, format_string ("%f", placingZone->cellsR3 [xx].height).c_str(),
					"u_ins", APIParT_CString, "벽세우기",
					"ang_x", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(),
					"ang_y", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str()));
			}
		...
		if ((placingZone->cellsLT [xx].objType == OUTPANEL) && (placingZone->cellsLT [xx].verLen > EPS) && (placingZone->cellsLT [xx].horLen > EPS) && (placingZone->cellsLT [xx].height > EPS))
			elemList.Push (outpanel.placeObject (4,
				"wid_s", APIParT_Length, format_string ("%f", placingZone->cellsLT [xx].verLen).c_str(),
				"leng_s", APIParT_Length, format_string ("%f", placingZone->cellsLT [xx].horLen).c_str(),
				"hei_s", APIParT_Length, format_string ("%f", placingZone->cellsLT [xx].height).c_str(),
		...
		if ((placingZone->cellsRT [xx].objType == OUTPANEL) && (placingZone->cellsRT [xx].verLen > EPS) && (placingZone->cellsRT [xx].horLen > EPS) && (placingZone->cellsRT [xx].height > EPS))
			elemList.Push (outpanel.placeObject (4,
				"wid_s", APIParT_Length, format_string ("%f", placingZone->cellsRT [xx].horLen).c_str(),
				"leng_s", APIParT_Length, format_string ("%f", placingZone->cellsRT [xx].verLen).c_str(),
				"hei_s", APIParT_Length, format_string ("%f", placingZone->cellsRT [xx].height).c_str(),
				"dir_s", APIParT_CString, "세우기"));
		...
		if ((placingZone->cellsLB [xx].objType == OUTPANEL) && (placingZone->cellsLB [xx].verLen > EPS) && (placingZone->cellsLB [xx].horLen > EPS) && (placingZone->cellsLB [xx].height > EPS))
			elemList.Push (outpanel.placeObject (4,
				"wid_s", APIParT_Length, format_string ("%f", placingZone->cellsLB [xx].horLen).c_str(),
				"leng_s", APIParT_Length, format_string ("%f", placingZone->cellsLB [xx].verLen).c_str(),
				"hei_s", APIParT_Length, format_string ("%f", placingZone->cellsLB [xx].height).c_str(),
				"dir_s", APIParT_CString, "세우기"));
		...
		if ((placingZone->cellsRB [xx].objType == OUTPANEL) && (placingZone->cellsRB [xx].verLen > EPS) && (placingZone->cellsRB [xx].horLen > EPS) && (placingZone->cellsRB [xx].height > EPS))
			elemList.Push (outpanel.placeObject (4,
				"wid_s", APIParT_Length, format_string ("%f", placingZone->cellsRB [xx].verLen).c_str(),
				"leng_s", APIParT_Length, format_string ("%f", placingZone->cellsRB [xx].horLen).c_str(),
				"hei_s", APIParT_Length, format_string ("%f", placingZone->cellsRB [xx].height).c_str(),
				"dir_s", APIParT_CString, "세우기"));
	}
...
GSErrCode	ColumnTableformPlacingZone::placeRestObjectsA_soleColumn (ColumnTableformPlacingZone* placingZone)
{
	...
	if (placingZone->bUseOutcornerPanel == true) {
		...
		elemList.Push (pipe.placeObject (7,
			...
			"p_leng", APIParT_Length, format_string ("%f", heightOfFormArea).c_str(),
	...
	if (placingZone->typeOfColumnBand == 1) {
		...
		elemList.Push (columnBand1.placeObject (4,
			...
			"c_w", APIParT_Length, format_string ("%f", placingZone->coreWidth + placingZone->venThick*2).c_str(),
			"c_h", APIParT_Length, format_string ("%f", placingZone->coreDepth + placingZone->venThick*2).c_str(),
			"addOffset", APIParT_Length, format_string ("%f", 0.050).c_str()));
...
GSErrCode	ColumnTableformPlacingZone::fillRestAreas_soleColumn (ColumnTableformPlacingZone* placingZone)
{
	...
	if (placingZone->bFillMarginTopAtNorth == true) {
		...
		if (placingZone->bExistBeams [NORTH] == true) {
			...
			moveIn3D ('z', placingZone->angle + DegreeToRad (180), heightOfFormArea + marginHeight, &plywood.posX, &plywood.posY, &plywood.posZ);
			if ((leftLenOfBeam > EPS) && (placingZone->areaHeight - heightOfFormArea - marginHeight > EPS)) {
				elemList.Push (plywood.placeObject (13,
					"p_stan", APIParT_CString, "비규격",
					"w_dir", APIParT_CString, "벽세우기",
					"p_thk", APIParT_CString, "11.5T",
					"p_wid", APIParT_Length, format_string ("%f", leftLenOfBeam).c_str(),
					"p_leng", APIParT_Length, format_string ("%f", placingZone->areaHeight - heightOfFormArea - marginHeight).c_str(),
			...
			if (marginHeight > EPS) {
				elemList.Push (plywood.placeObject (13,
					...
					"w_dir", APIParT_CString, "벽세우기",
					"p_thk", APIParT_CString, "11.5T",
					"p_wid", APIParT_Length, format_string ("%f", columnWidth + 0.0615).c_str(),
					"p_leng", APIParT_Length, format_string ("%f", marginHeight).c_str(),
					"p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(),
					"sogak", APIParT_Boolean, "1.0",
					"bInverseSogak", APIParT_Boolean, "1.0",
					"prof", APIParT_CString, "소각",
			...
			moveIn3D ('z', placingZone->angle + DegreeToRad (180), heightOfFormArea + marginHeight, &plywood.posX, &plywood.posY, &plywood.posZ);
			if ((rightLenOfBeam + 0.0615 > EPS) && (placingZone->areaHeight - heightOfFormArea - marginHeight > EPS)) {
				elemList.Push (plywood.placeObject (13,
					"p_stan", APIParT_CString, "비규격",
					"w_dir", APIParT_CString, "벽세우기",
					"p_thk", APIParT_CString, "11.5T",
					"p_wid", APIParT_Length, format_string ("%f", rightLenOfBeam + 0.0615).c_str(),
					"p_leng", APIParT_Length, format_string ("%f", placingZone->areaHeight - heightOfFormArea - marginHeight).c_str(),
					"p_ang", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(),
					"sogak", APIParT_Boolean, "1.0",
					"bInverseSogak", APIParT_Boolean, "1.0",
	...
	if (placingZone->bFillMarginTopAtSouth == true) {
		...
		if (placingZone->bExistBeams [SOUTH] == true) {
			...
			moveIn3D ('z', placingZone->angle, heightOfFormArea + marginHeight, &plywood.posX, &plywood.posY, &plywood.posZ);
			if ((leftLenOfBeam > EPS) && (placingZone->areaHeight - heightOfFormArea - marginHeight > EPS)) {
				elemList.Push (plywood.placeObject (13,
					"p_stan", APIParT_CString, "비규격",
					"w_dir", APIParT_CString, "벽세우기",
	...
	if (placingZone->bFillMarginTopAtWest == true) {
		...
		if (placingZone->bExistBeams [WEST] == true) {
			...
			moveIn3D ('z', placingZone->angle - DegreeToRad (90), heightOfFormArea + marginHeight, &plywood.posX, &plywood.posY, &plywood.posZ);
			if ((leftLenOfBeam > EPS) && (placingZone->areaHeight - heightOfFormArea - marginHeight > EPS)) {
				elemList.Push (plywood.placeObject (13,
					"p_stan", APIParT_CString, "비규격",
					"w_dir", APIParT_CString, "벽세우기",
...
short DGCALLBACK columnTableformPlacerHandler_soleColumn_2 (short message, short dialogID, short item, DGUserData /* userData */, DGMessageData /* msgData */)
{
	...
	switch (message) {
		case DG_MSG_INIT:
			...
			// 여백 계산 (북)
			if (placingZone.bExistBeams [NORTH] == true)
				placingZone.marginTopAtNorth = placingZone.bottomLevelOfBeams [NORTH] - placingZone.baseElev;
			else
			...
			// 여백 계산 (남)
			if (placingZone.bExistBeams [SOUTH] == true)
				placingZone.marginTopAtSouth = placingZone.bottomLevelOfBeams [SOUTH] - placingZone.baseElev;
			else
			...
			// 여백 계산 (서)
			if (placingZone.bExistBeams [WEST] == true)
				placingZone.marginTopAtWest = placingZone.bottomLevelOfBeams [WEST] - placingZone.baseElev;
			else
			...
			// 여백 계산 (동)
			if (placingZone.bExistBeams [EAST] == true)
				placingZone.marginTopAtEast = placingZone.bottomLevelOfBeams [EAST] - placingZone.baseElev;
			else
