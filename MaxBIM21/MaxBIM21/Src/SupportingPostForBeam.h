...
namespace SupportingPostForBeam {
	...
	void placePERISupports ()
	{
		...
		if (bMRK1 == true) {
			...
			elemList.Push (PERI.placeObject (14, "ZZYZX", APIParT_Length, "4.3", "stType", APIParT_CString, "MP 625", "bCrosshead", APIParT_Boolean, "1.0", "posCrosshead", APIParT_CString, "하단", "crossheadType", APIParT_CString, "PERI", "angCrosshead", APIParT_Angle, format_string ("%f", DegreeToRad (90.0)).c_str(), "len_min", APIParT_Length, "4.3", "len_max", APIParT_Length, "6.25", "len_current", APIParT_Length, "4.3", "pos_lever", APIParT_Length, "4.220", "angX", APIParT_Angle, format_string ("%f", DegreeToRad (0.0)).c_str(), "angY", APIParT_Angle, format_string ("%f", DegreeToRad (180.0)).c_str(), "bShowCoords", APIParT_Boolean, "0.0", "text2_onoff", APIParT_Boolean, "0.0"));
	...
	void placeMRKs ()
	{
		...
		if (bMRK1 == true) {
			...
			elemList.Push (MRK.placeObject (9, "A", APIParT_Length, format_string ("%f", parallel_len - 0.100).c_str(), "B", APIParT_Length, "0.100", "ZZYZX", APIParT_Length, "0.500", "stType", APIParT_CString, supportingPostPlacingZone.MRK_Type_1_parallel.ToCStr ().Get (), "diaVerticalPipe", APIParT_Length, "0.100", "lenFrame", APIParT_Length, format_string ("%f", parallel_len - 0.100).c_str(), "angX", APIParT_Angle, "0.0", "angY", APIParT_Angle, "0.0", "bOnlyCoupler", APIParT_Boolean, "0.0"));
			// 오른쪽
			...
			elemList.Push (MRK.placeObject (9, "A", APIParT_Length, format_string ("%f", perpendicular_len - 0.100).c_str(), "B", APIParT_Length, "0.100", "ZZYZX", APIParT_Length, "0.500", "stType", APIParT_CString, supportingPostPlacingZone.MRK_Type_1_perpendicular.ToCStr ().Get (), "diaVerticalPipe", APIParT_Length, "0.100", "lenFrame", APIParT_Length, format_string ("%f", perpendicular_len - 0.100).c_str(), "angX", APIParT_Angle, "0.0", "angY", APIParT_Angle, "0.0", "bOnlyCoupler", APIParT_Boolean, "0.0"));
			MRK.radAng -= DegreeToRad (90.0);
			// 위쪽
	...
	switch (message) {
		case DG_MSG_INIT:
			...
			for (int i = 0 ; i <= (storyInfo.lastStory - storyInfo.firstStory) ; ++i) {
				sprintf (floorIndex, "%d", storyInfo.data [0][i].index);
				GS::UniString UniFloorIndex = floorIndex;
				GS::UniString UniFloorName = storyInfo.data[0][i].uName;
				GS::UniString UniFloorFullName = UniFloorIndex + ". " + UniFloorName;
				DGPopUpInsertItem (dialogID, POPUP_FLOOR, DG_POPUP_BOTTOM);
				DGPopUpSetItemText (dialogID, POPUP_FLOOR, DG_POPUP_BOTTOM, UniFloorFullName);
			}
